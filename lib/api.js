...
// ── Scoring algorithm ─────────────────────────────────────────
export function scoreSlots(slots, { sortMode = 'default', cargo_weight_kg = 24000, requiredArrival = null } = {}) {
  if (!slots.length) return [];

  const ROAD_CO2 = 0.096;
  const requiredBy = requiredArrival ? new Date(requiredArrival) : null;
  ...
  const withMetrics = slots.map(s => {
    ...
    // Check if slot meets required delivery date
    const meetsDeadline = requiredBy ? arr <= requiredBy : true;
    return { ...s, transitH, totalPrice, co2Saved, meetsDeadline };
  });
  ...
  const scored = withMetrics.map(s => ({
    ...
    score: (weights.price * norm(s.totalPrice, prices)
         ...
         // Penalise slots that miss the required delivery date heavily
         * (s.meetsDeadline ? 1 : 0.01),
    missesDeadline: !s.meetsDeadline,
  }));
